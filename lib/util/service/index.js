var observer = require('asgc-observer');
...
Service.prototype.defineAction = function(url,method,callback){
...
Service.prototype.http = function(url,method,req,resp,data,callback,error){
	...
	if(this.observer.test(fullPath)){
		result = this.observer.trigger(fullPath,req,resp,params);
	}else if((!method || 'GET' == method) && this.observer.test(path)){
		result = this.observer.trigger(path,req,resp,params);
	}else if(!method && this.observer.test(path + '.' + 'GET')){
		result = this.observer.trigger(path + '.' + 'GET',req,resp,params);
	}else{
		error && error(404);
		return 404;
	}
